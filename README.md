## Deploy

GitHub Pages auto-publishes `main` to https://qishao11.github.io after every push.

## Customize

- `index.html` — edit homepage content here: profile, links, News, and Representative Academic Work
- `about.html` — edit Education, Research Experience, Honors, and Academic Service
- `publications.html` — edit selected publications and patents
- `stylesheet.css` — edit visual style here: layout, spacing, colors, mobile behavior
- `styles.css` — optional small overrides, loaded after `stylesheet.css`
- `images/Personal.jpg` — your photo
- `cv.pdf` — drop your CV here

To replace the profile image, drop a JPG at `images/Personal.jpg` and git push.

## Fonts

This site uses HarmonyOS Sans Fonts. The files under `fonts/` are unmodified
copies redistributed under the HarmonyOS Sans Fonts License Agreement, kept at
`fonts/HarmonyOS-Sans-LICENSE.txt`. The license forbids modifying the fonts, so
they are served as the original TTFs — do not subset them or convert them to
WOFF2.
